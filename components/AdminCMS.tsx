...
import { Project, BlogPost, Service, TestimonialItem } from '../types';
...
type CMSTab = 'overview' | 'projects' | 'blogs' | 'website' | 'services' | 'testimonials' | 'settings';
    ...
    <div className="pt-24 pb-32 min-h-screen bg-gray-950 text-gray-100 transition-colors">
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        <div className="flex items-center gap-2 pb-4 overflow-x-auto no-scrollbar border-b border-gray-800/80 mb-8">
          {[
            { id: 'overview' as const, label: 'Dashboard', icon: '📊', count: null },
            { id: 'projects' as const, label: 'Projects & Case Studies', icon: '💼', count: projects.length },
            { id: 'blogs' as const, label: 'Tech News & Blog', icon: '📰', count: blogPosts.length },
            { id: 'website' as const, label: 'Website Content & Banner', icon: '🌐', count: null },
            { id: 'services' as const, label: 'Services', icon: '🛠️', count: services.length },
            { id: 'testimonials' as const, label: 'Testimonials', icon: '⭐', count: testimonials.length },
            { id: 'settings' as const, label: 'Settings & Backups', icon: '⚙️', count: null },
          ].map(tab => (
            <button
              key={tab.id}
              onClick={() => {
          ...
          <div className="space-y-8 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="p-6 rounded-3xl bg-gray-900 border border-gray-800 relative overflow-hidden">
                ...
                <p className="text-xs text-gray-400">{services.length} active service capabilities</p>
                <button
